...
export class AppService {
  ...
  async getInfo(url: string): Promise<any> {
      ...
      this.httpService
        ...
        .pipe(
          ...
          mergeMap<string, Observable<SiteInfo>>((page) => {
            ...
            siteInfo.description = $('meta[name="description"]', 'head').attr(
              'content',
            );
    ...
    return { statusCode: HttpStatus.OK, message: '获取站点信息成功', result};
  }
      ...
      $(selector).each((i, elem: Element) => {
        ...
        if (href && href !== '#') {
          ...
          if (
            ...
            href.startsWith('apple') ||
            href.startsWith('safari')
          ) {
            href = `${siteInfo.origin}/${href}`;
          }
  ...
  private async browserconfig($: cheerio.CheerioAPI): Promise<Icon[]> {
    ...
    const tileImage = $('meta[name="msapplication-TileImage"]', 'head').attr(
      'content',
    );
  ...
  private async manifest(
    ...
    const result = await lastValueFrom(
      this.httpService.get(url, { proxy: false }),
    );
    if (result.status == 200) {
      const data = result.data;
      if (isArray(data.icons)) {
        icons =
          data.icons.map(({ src, sizes, type }) => {
            if (src.startsWith('/') && !src.startsWith('//'))
              src = `${siteInfo.origin}${src}`;
  ...
  private async favicon($: cheerio.CheerioAPI): Promise<Icon[]> {
    ...
    icons = await lastValueFrom(
      this.httpService
        .get(src, { proxy: false, responseType: 'arraybuffer' })
        .pipe(
          map((response) => {
            if (response.status == 200) {
